...
-- Dumping database structure for sampledata
CREATE DATABASE IF NOT EXISTS `sampledata` /*!40100 DEFAULT CHARACTER SET armscii8 COLLATE armscii8_bin */ /*!80016 DEFAULT ENCRYPTION='N' */;
USE `sampledata`;

-- Dumping structure for table sampledata.customers
...
-- Dumping data for table sampledata.customers: ~122 rows (approximately)
...
-- Dumping structure for table sampledata.employees
...
-- Dumping data for table sampledata.employees: ~23 rows (approximately)
...
-- Dumping structure for table sampledata.offices
...
-- Dumping data for table sampledata.offices: ~7 rows (approximately)
...
-- Dumping structure for table sampledata.orderdetails
...
-- Dumping data for table sampledata.orderdetails: ~2,996 rows (approximately)
...
-- Dumping structure for table sampledata.orders
CREATE TABLE IF NOT EXISTS `orders` (
  ...
  KEY `date_order` (`orderDate`),
  CONSTRAINT `orders_ibfk_1` FOREIGN KEY (`customerNumber`) REFERENCES `customers` (`customerNumber`)
) ENGINE=InnoDB DEFAULT CHARSET=latin1;

-- Dumping data for table sampledata.orders: ~326 rows (approximately)
...
-- Dumping structure for table sampledata.payments
...
-- Dumping data for table sampledata.payments: ~273 rows (approximately)
...
-- Dumping structure for table sampledata.productlines
...
-- Dumping data for table sampledata.productlines: ~7 rows (approximately)
...
-- Dumping structure for table sampledata.products
...
-- Dumping data for table sampledata.products: ~110 rows (approximately)
...
-- Dumping structure for view sampledata.test
-- Creating temporary table to overcome VIEW dependency errors
CREATE TABLE `test` (
	`orderDate` DATE NOT NULL,
	`status` VARCHAR(15) NOT NULL COLLATE 'latin1_swedish_ci',
	`customerName` VARCHAR(50) NULL COLLATE 'latin1_swedish_ci'
) ENGINE=MyISAM;

-- Dumping structure for view sampledata.test
-- Removing temporary table and create final VIEW structure
DROP TABLE IF EXISTS `test`;
CREATE ALGORITHM=UNDEFINED SQL SECURITY DEFINER VIEW `test` AS select `od`.`orderDate` AS `orderDate`,`od`.`status` AS `status`,`cs`.`customerName` AS `customerName` from (`orders` `od` left join `customers` `cs` on((`cs`.`customerNumber` = `od`.`customerNumber`))) order by `od`.`orderNumber` desc limit 4;
